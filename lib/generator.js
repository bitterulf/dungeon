...
var Generator = function (data) {
  ...
  this.addAreaCount = function (min, max) {
    this.validators.push({
      command: 'areaCount',
      min: min,
      max: max
  ...
  this.areaCount = function() {
    ...
    for (var x = 0; x < this.width; x++) {
      for (var y = 0; y < this.height; y++) {
        if (this.data[y * this.width + x] == sourceId && _.indexOf(globalVisited, y * this.width + x) == -1) {
          id = y * this.width + x;
          visited = [];
          todo = [];
          ...
          if (x-1 >=0 && this.data[y * this.width + (x - 1)] == sourceId && _.indexOf(globalVisited, y * this.width + (x-1)) == -1) {
            todo.push({y: y, x: x-1});
            globalVisited.push(y * this.width + (x-1));
          }

          if (y-1 >=0 && this.data[(y-1) * this.width + x] == sourceId && _.indexOf(globalVisited, (y-1) * this.width + x) == -1) {
            todo.push({y: y-1, x: x});
            globalVisited.push((y-1) * this.width + x);
          }

          if (y+1 <this.height && this.data[(y+1) * this.width + x] == sourceId && _.indexOf(globalVisited, (y+1) * this.width + x) == -1) {
            todo.push({y: y+1, x: x});
            globalVisited.push((y+1) * this.width + x);
          }

          if (x+1 <this.width && this.data[y * this.width + (x + 1)] == sourceId && _.indexOf(globalVisited, y * this.width + (x+1)) == -1) {
            todo.push({y: y, x: x+1});
            globalVisited.push(y * this.width + (x+1));
          }
          ...
          while (todo.length) {
            ...
            if (t.x-1 >=0 && this.data[t.y * this.width + (t.x - 1)] == sourceId && _.indexOf(globalVisited, t.y * this.width + (t.x-1)) == -1) {
              todo.push({y: t.y, x: t.x-1});
              globalVisited.push(t.y * this.width + (t.x-1));
            }
            if (t.y-1 >=0 && this.data[(t.y-1) * this.width + t.x] == sourceId && _.indexOf(globalVisited, (t.y-1) * this.width + t.x) == -1) {
              todo.push({y: t.y-1, x: t.x});
              globalVisited.push((t.y-1) * this.width + t.x);
            }

            if (t.y+1 <this.height && this.data[(t.y+1) * this.width + t.x] == sourceId && _.indexOf(globalVisited, (t.y+1) * this.width + t.x) == -1) {
              todo.push({y: t.y+1, x: t.x});
              globalVisited.push((t.y+1) * this.width + t.x);
            }

            if (t.x+1 <this.width && this.data[t.y * this.width + (t.x + 1)] == sourceId && _.indexOf(globalVisited, t.y * this.width + (t.x+1)) == -1) {
              todo.push({y: t.y, x: t.x+1});
              globalVisited.push(t.y * this.width + (t.x+1));
            }
          }
